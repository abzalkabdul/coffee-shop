...
class MenuView(generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['drinks_categories'] = Drinks.objects.values_list(
            'category', flat=True).distinct()
        drinks_categories_with_image = {}
        for category in context['drinks_categories']:
            item = Drinks.objects.filter(
                category=category, image__isnull=False).first()
        ...
        context['food_categories'] = Food.objects.values_list(
            'category', flat=True).distinct()
        food_categories_with_image = {}
        for category in context['food_categories']:
            item = Food.objects.filter(
                category=category, image__isnull=False).first()
...
def menu_specified(request, category):
    ...
    drinks_categories = Drinks.objects.values_list(
        'category', flat=True).distinct()
    food_categories = Food.objects.values_list(
        'category', flat=True).distinct()
    ...
    if request.method == "POST":
        ...
        if action in ("decrease", "increase", "delete_item"):
            ...
            if action == "delete_item":
                get_item = get_object_or_404(Cart, id=item_id)
                get_item.delete()
            elif action == "decrease":
                if get_item.quantity >= 2:
                    get_item.quantity -= 1
                    get_item.save()
            ...
            specified_item_category = request.POST.get(
                "specified_item_category")
...
def after_payment(request):
    return render(request, 'after_payment.html')